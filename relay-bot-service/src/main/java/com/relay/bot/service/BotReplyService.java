 ...
 * Runs asynchronously so the RabbitMQ listener thread is released immediately.
 */
@Service
public class BotReplyService {
    ...
    public BotReplyService(MessageServiceClient messageServiceClient, BotProperties botProperties) {
        this.messageServiceClient = messageServiceClient;
        this.botProperties = botProperties;
    }

    /**
     * Posts a reply to the channel the event arrived on after the configured delay.
    ...
    public void reply(MessagePublishedEvent event) {
        ...
        try {
            Thread.sleep(botProperties.replyDelayMs());
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            log.warn("Reply delay interrupted for messageId={}", event.messageId());
            return;
        }
